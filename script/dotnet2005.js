...
function hideBoxes()
{
    ...
    if (pres)
    {
        for (var iPre = 0; iPre < pres.length; iPre++)
        {
            ...
            if (pre.className)
            {
                if (pre.className == "dxpopupbubble")
                {
...
function bodyLoadCommonBefore()
{
    ...
    //IE crashes if window is closed and we try to access window.external
    //so if IE we attach to onbeforeunload method
...
function resolveHelp2Keyword(Keyword,OnlineKeyword)
{
	...
	return "http://msdn2.microsoft.com/en-us/library/" + OnlineKeyword + ".aspx"
}
